...
def relatorio():
    conn = conectar_banco()
    # Busca todas as máquinas registradas no banco de dados SQLite
    frota_db = conn.execute('SELECT * FROM frota').fetchall()
    conn.close()

    total_maquinas = len(frota_db)
    
    # Contagem inteligente baseada nos dados reais do banco
    em_alerta = sum(1 for m in frota_db if m['status'] == "Manutenção Requerida")
    operacionais = total_maquinas - em_alerta
    
    # Cálculo de porcentagem de disponibilidade
